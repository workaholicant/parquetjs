...
class ParquetEnvelopeReader {
  ...
  async search(paths, filter, _rowGroups) {
    ...
    await Promise.all(rowGroups.map(async rowGroup => {
      ...
      return await Promise.all(pages.map( async (pageNo) => {
        if (excludedPages.has(pageNo)) return;
        let pageRecords = [];
        await Promise.all(paths.map(async (path,i) => {
          pageRecords = await this.readPage(offsets[i],pageNo, pageRecords);
        }));
