...
const RangeReader = require('./rangeReader');
...
class ParquetEnvelopeReader {
  ...
  static async openUrl(request, params, options) {
    ...
    base = base.slice(0,base.length-2).join('/');
    ...
    if (options.rangeSize) {
      let readers = {};
      readFn = (offset, length, file) => {
        let url = file ? base + file : params.url;
        const reader = readers[url] || (readers[url] = new RangeReader(request, url, params, options));
