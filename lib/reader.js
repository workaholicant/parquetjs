...
class ParquetEnvelopeReader {
  ...
  search(paths, options) {
    ...
    async function results(page) {
      ...
      /* find the first matching value for a path from a record */
      function getValue(path,record) {
        if (typeof path === 'string') {
          path = path.split(',');
        }
        const next = path.shift();
        /* If we hit an array we take the first element */
        if (Array.isArray(record)) {
          record = record[0];
        }
        record = record[next];
        if (path.length > 0 && record !== undefined) {
          return getValue(path,record);
        }
        return record;
      }
      ...
      pageRecords.forEach(rec => {
        if (paths.every(path => {
          const value = getValue(path.path, rec);
          return (path.index && !sourceIncluded) || (
            (path.min === undefined || value >= path.min) &&
            (path.max === undefined || value <= path.max) &&
            (path.value === undefined || value == path.value) &&
