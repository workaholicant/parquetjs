...
class ParquetWriter {
  ...
  appendRow(row) {
    ...
    return _asyncToGenerator(function* () {
      ...
      parquet_shredder.shredRecord(_this.schema, row, _this.rowBuffer);

      if (_this.rowBuffer.pageRowCount > _this.envelopeWriter.pageSize) {
        encodePages(_this.schema, _this.rowBuffer, { useDataPageV2: _this.envelopeWriter.useDataPageV2 });
      }

      if (_this.rowBuffer.rowCount >= _this.rowGroupSize) {
        encodePages(_this.schema, _this.rowBuffer, { useDataPageV2: _this.envelopeWriter.useDataPageV2 });
        yield _this.envelopeWriter.writeRowGroup(_this.rowBuffer);
        _this.rowBuffer = {};
      }
    })();
  }
  ...
  close(callback) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (_this2.rowBuffer.rowCount > 0 || _this2.rowBuffer.rowCount >= _this2.rowGroupSize) {
        encodePages(_this2.schema, _this2.rowBuffer, { useDataPageV2: _this2.envelopeWriter.useDataPageV2 });
        yield _this2.envelopeWriter.writeRowGroup(_this2.rowBuffer);
        _this2.rowBuffer = {};
      }

      yield _this2.envelopeWriter.writeIndex();
      yield _this2.envelopeWriter.writeFooter(_this2.userMetadata);
      yield _this2.envelopeWriter.close();
      _this2.envelopeWriter = null;
...
function encodePages(schema, rowBuffer, opts) {
  ...
  for (let field of schema.fieldList) {
    ...
    if (opts.useDataPageV2) {
      page = encodeDataPageV2(field, values.count, rowBuffer.pageRowCount, values.values, values.rlevels, values.dlevels, statistics);
    } else {
      page = encodeDataPage(field, values.count, values.values, values.rlevels, values.dlevels, statistics);
    }
...
function encodeDataPageV2(column, valueCount, rowCount, values, rlevels, dlevels, statistics) {
  ...
  /* concat page header, repetition and definition levels and values */
  return [parquet_util.serializeThrift(pageHeader), rLevelsBuf, dLevelsBuf, valuesBufCompressed];
}
...
function encodeColumnChunk(pages, opts) {
  ...
  metadata.codec = parquet_thrift.CompressionCodec[opts.useDataPageV2 ? opts.column.compression : 'UNCOMPRESSED'];
